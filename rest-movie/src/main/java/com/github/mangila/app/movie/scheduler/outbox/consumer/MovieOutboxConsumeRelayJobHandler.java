...
public class MovieOutboxConsumeRelayJobHandler implements JobRequestHandler<MovieOutboxConsumeRelayJobRequest> {
    ...
    public MovieOutboxConsumeRelayJobHandler(MovieProperties movieProperties, TransactionTemplate transactionTemplate, MovieOutboxService movieOutboxService, MovieScheduler movieScheduler, MovieHistoryService movieHistoryService) {
        ...
        this.destinationMap = Map.of(
                HTTP, projection -> movieScheduler.schedule(new MovieHttpDestinationJobRequest(projection.payload())),
                KAFKA, projection -> movieScheduler.schedule(new MovieKafkaDestinationJobRequest(projection.payload())),
                RABBITMQ, projection -> movieScheduler.schedule(new MovieHttpDestinationJobRequest(projection.payload())),
                SQS, projection -> movieScheduler.schedule(new MovieHttpDestinationJobRequest(projection.payload()))
        );
    }
    ...
    public void run(MovieOutboxConsumeRelayJobRequest jobRequest) throws Exception {
        ...
        var claimStep = context.runStepOnce("claimDestinationPending", () -> {
        ...
        for (var destinationEntity : destinationProjections) {
            ...
            var payloadStep = context.runStepOnce("payload:" + idAsString, () -> {
            ...
            context.runStepOnce("schedule:" + idAsString, () -> {
                ...
                final var jobId = destinationMap.get(destination)
                        .apply(payload);
