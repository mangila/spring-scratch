...
public class MovieOutboxRelayJobHandler implements JobRequestHandler<MovieOutboxRelayJobRequest> {
	...
	public MovieOutboxRelayJobHandler(ClaimBatchStepHandler claimBatchStepHandler,
			ClaimVersionStepHandler claimVersionStepHandler, CreateDestinationStepHandler createDestinationStepHandler,
	...
	public void run(MovieOutboxRelayJobRequest jobRequest) throws Exception {
		...
		final ClaimBatchStepResult claimBatchStepResult = context.runStepOnce("claimBatch",
				() -> claimBatchStepHandler.handle(limit));
		...
		for (var outbox : outboxProjections) {
			...
			try {
				final boolean versionOk = context.runStepOnce("claimVersion:" + outboxId,
						() -> claimVersionStepHandler.handle(outbox));
				if (!versionOk) {
					log.warn("Version mismatch for outbox id: {}", outboxId);
					errs.add(outboxId);
					continue;
				}
				final CreateDestinationStepResult createDestinationStepResult = context
					.runStepOnce("destination:" + outboxId, () -> createDestinationStepHandler.handle(outboxId));
				final FetchPayloadStepResult fetchPayloadStepResult = context.runStepOnce("payload:" + outboxId,
						() -> fetchPayloadStepHandler.handle(historyId));
				final var payload = fetchPayloadStepResult.projection().payload();
				...
				// context.runStepOnce("updateVersion:" + outboxId, () ->
				// updateVersionStepHandler.handle(outbox));
			}
			catch (Exception e) {
