...
public class DirectorGridView extends VerticalLayout {
	...
	public DirectorGridView(DirectorService directorService) {
		...
		grid.addColumn(iconRenderer()).setHeader("Director").setFlexGrow(0);
		grid.addColumn(DirectorProjection::name).setHeader("Name").setSortable(true);
		grid.addColumn(DirectorProjection::name).setHeader("Date of Birth").setSortable(true);
		grid.addColumn(DirectorProjection::biography).setHeader("Bio");
